...
public class Controller_RelatorioTratamento implements Initializable {
	private ObservableList<String> cb;

	@FXML
	private TableView<TratamentoVO> lista;
	@FXML
	private TableColumn<TratamentoVO, String> codigo;
	@FXML
	private TableColumn<TratamentoVO, Long> id;
	@FXML
	private TableColumn<TratamentoVO, String> data;
	...
	public void initialize(URL arg0, ResourceBundle arg1) {
		...
		ObservableList<TratamentoVO> clientes = FXCollections.observableArrayList(aux.listar());
		codigo.setCellValueFactory(new PropertyValueFactory<TratamentoVO, String>("codigo"));
		data.setCellValueFactory(new PropertyValueFactory<TratamentoVO, String>("string"));
		id.setCellValueFactory(new PropertyValueFactory<TratamentoVO, Long>("id"));
		...
		if ((escolha.getSelectionModel().getSelectedItem() != null)) {
			if (escolha.getSelectionModel().getSelectedItem().equals("Data")) {
				if (comeco.getValue() != null) {
					...
					ObservableList<TratamentoVO> tratamentos = FXCollections.observableArrayList(aux.relatorio(com, fi));
					codigo.setCellValueFactory(new PropertyValueFactory<TratamentoVO, String>("codigo"));
					data.setCellValueFactory(new PropertyValueFactory<TratamentoVO, String>("string"));
					id.setCellValueFactory(new PropertyValueFactory<TratamentoVO, Long>("id"));
					lista.setItems(tratamentos);
					mensagem.setTextFill(Color.web("green"));
					if(dataInicio.equals(dataFim)) {
						mensagem.setText("Mostrando relatório de tratamentos feitos em " + dataInicio);
					} else {
						mensagem.setText("Mostrando relatório de tratametnos feitos de " + dataInicio + " até " + dataFim);
					}
				...
				if (!pesquisa.getText().isEmpty()) {
					...
					ObservableList<TratamentoVO> tratamentos = FXCollections.observableArrayList(aux.listarCliente(pesquisa.getText()));
					codigo.setCellValueFactory(new PropertyValueFactory<TratamentoVO, String>("codigo"));
					data.setCellValueFactory(new PropertyValueFactory<TratamentoVO, String>("string"));
					id.setCellValueFactory(new PropertyValueFactory<TratamentoVO, Long>("id"));
					lista.setItems(tratamentos);
				...
				if (!pesquisa.getText().isEmpty()) {
					mensagem.setVisible(false);
					ObservableList<TratamentoVO> tratamentos = FXCollections.observableArrayList(aux.listarFuncionario(pesquisa.getText()));
					codigo.setCellValueFactory(new PropertyValueFactory<TratamentoVO, String>("codigo"));
					data.setCellValueFactory(new PropertyValueFactory<TratamentoVO, String>("string"));
					id.setCellValueFactory(new PropertyValueFactory<TratamentoVO, Long>("id"));
					lista.setItems(tratamentos);
	...
	public void notaFiscal(ActionEvent event) throws IOException {
		TratamentoVO aux = lista.getSelectionModel().getSelectedItem();
		if (aux != null) {
